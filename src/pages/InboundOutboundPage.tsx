...
import {
  ...
  Sparkles, Clock, Gauge, LayoutDashboard,
} from 'lucide-react';
...
function ClinicDashboardMock() {
  ...
  const appTabs = t('dashboard.appTabs', { returnObjects: true }) as string[];
  const stats = t('dashboard.stats', { returnObjects: true }) as StatTile[];
  const rows = t('dashboard.recent.rows', { returnObjects: true }) as CallRow[];
  const agents = t('dashboard.agents.items', { returnObjects: true }) as string[];
  const bookings = t('dashboard.bookings.items', { returnObjects: true }) as BookingRow[];
    ...
    <div className="rounded-3xl overflow-hidden shadow-2xl border border-ink-200 bg-white">
      ...
      <div className="bg-ink-50 p-4 sm:p-6 space-y-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h3 className="text-2xl font-bold text-ink-900">{t('dashboard.heading')}</h3>
            <p className="text-sm text-ink-600 mt-0.5">{t('dashboard.headingSub')}</p>
          </div>
          <div className="text-left sm:text-right">
            <div className="text-sm font-semibold text-ink-800">{t('dashboard.org')}</div>
            <div className="text-xs text-ink-400">{t('dashboard.period')}</div>
          </div>
        </div>
                ...
                {(['agents', 'calls', 'minutes'] as const).map((metric) => (
                  <div key={metric}>
                    <div className="text-xl font-bold text-ink-900">
                      {t(`dashboard.panels.${key}.${metric}`)}
                    </div>
                    <div className="text-[11px] text-ink-400 mt-0.5">
                      {t(`dashboard.panels.${metric}Label`)}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="grid lg:grid-cols-3 gap-3 sm:gap-4 items-start">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-ink-200 overflow-hidden">
            <h4 className="font-semibold text-ink-900 px-5 pt-5 pb-3">{t('dashboard.recent.title')}</h4>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[520px]">
                <thead>
                  <tr className="bg-ink-50 text-[11px] uppercase tracking-wider text-ink-400">
                    <th scope="col" className="text-left font-semibold px-5 py-2.5">{t('dashboard.recent.columns.agent')}</th>
                    <th scope="col" className="text-left font-semibold px-3 py-2.5">{t('dashboard.recent.columns.date')}</th>
                    <th scope="col" className="text-left font-semibold px-3 py-2.5">{t('dashboard.recent.columns.status')}</th>
                    <th scope="col" className="text-left font-semibold px-3 py-2.5">{t('dashboard.recent.columns.minutes')}</th>
          ...
          <div className="space-y-3 sm:space-y-4">
            <div className="bg-white rounded-2xl border border-ink-200 p-5">
              <h4 className="font-semibold text-ink-900 mb-3">{t('dashboard.agents.title')}</h4>
            ...
            <div className="bg-white rounded-2xl border border-ink-200 p-5">
              <h4 className="font-semibold text-ink-900 mb-3">{t('dashboard.bookings.title')}</h4>
              <ul className="space-y-3">
    ...
    <div className="min-h-screen bg-white">
      ...
      <section id="dashboard" className="py-20 md:py-28 bg-white scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-center text-sm text-ink-400 mt-6">{t('dashboard.note')}</p>
        </div>
      </section>
